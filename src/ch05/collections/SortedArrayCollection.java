...
public class SortedArrayCollection<T extends Comparable<T>> implements CollectionInterface<T>  
{
  ...
  // Adds element to this collection.
  {
    ...
    //shift all elements right by one from added location to the end 
    //  start at the end so temporary pointer is not needed
...
//toString method.
  {
	  ...
	  for (int i = 0; i < numElements; i++)
	  {
		  finalString += elements[i].toString();
	  }
	  return finalString;
  }
...
//returns the smallest element of the collection.
  {
	  ...
	  {
		  if ( elements[smallest].compareTo(elements[i]) < 0)
				smallest = i;
	  }
...
//this object and the argument object.
  {
		SortedArrayCollection<T> newSAC;
		newSAC = new SortedArrayCollection<T>();
		for (int i = 0; i < other.size(); i++)
		{
			newSAC.add(other.elements[i]);
		}
		for (int i = 0; i < numElements; i++)
		{
			newSAC.add(elements[i]);
		}
		return newSAC; 
  }
}
