...
public class ArrayCollection<T> implements CollectionInterface<T>  
{
  ...
  {
	//creates and returns a new ArrayCollection object
	//that is a combination of this object and the argument object.
	ArrayCollection<T> newAC;
	newAC = new ArrayCollection<T>(this.size() + other.size());
	for (int i = 0; i < other.size(); i++)
	{
		newAC.add(other.elements[i]);
	}
	for (int i = 0; i < numElements; i++)
	{
		newAC.add(elements[i]);
	}
	return newAC; 
  }
